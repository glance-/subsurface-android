...
if [ ! -e libusb-1.0.18.tar.bz2 ] ; then
	wget http://sourceforge.net/projects/libusb/files/libusb-1.0/libusb-1.0.18/libusb-1.0.18.tar.bz2
fi
if [ ! -e libusb-1.0.18 ] ; then
	tar -jxf libusb-1.0.18.tar.bz2
fi
if [ ! -e $PKG_CONFIG_PATH/libusb-1.0.pc ] ; then
	mkdir -p libusb-build-$ARCH
	pushd libusb-build-$ARCH
	../libusb-1.0.18/configure --host=${BUILDCHAIN} --prefix=${PREFIX} --enable-static --disable-shared --disable-udev
	make
	make install
	popd
	# Bugfix stupid -c flag
	perl -pi -e 's/^(Libs.private:.*)-c(.*)$/$1$2/' $PKG_CONFIG_PATH/libusb-1.0.pc
fi
